    ...
    public class Game
    {
        ...
        public void Start()
        {
            ...
            while (true)
            {
                ...
                if (CheckCommand(command, new[] { "?", "help", "commands" }))
                {
                    ShowHelp();
                }
            }
        ...
        private static void ShowItem(Item item)
        {
            ...
            if (item.Energy > 0)
            {
                ConsoleWriter.WriteText("\t\tEnergy: ", ConsoleColor.White, false);
                ConsoleWriter.WriteText($"{item.Energy}");
            }
            if (item.ExploitEfficiency > 0)
            {
                ConsoleWriter.WriteText("\t\tExploitEfficiency: ", ConsoleColor.White, false);
                ConsoleWriter.WriteText($"{item.ExploitEfficiency}");
            }
            if (item.Security > 0)
            {
                ConsoleWriter.WriteText("\t\tSecurity: ", ConsoleColor.White, false);
                ConsoleWriter.WriteText($"{item.Security}");
            }
        ...
        private static void ShowPlayerItem(PlayerItem playerItem)
        {
            ...
            if (playerItem.Item.Energy > 0)
            {
                ConsoleWriter.WriteText($"\t\tEnergy: {playerItem.RemainingEnergy}/{playerItem.Item.Energy}");
            }
            if (playerItem.Item.ExploitEfficiency > 0)
            {
                ConsoleWriter.WriteText($"\t\tExploitEfficiency: {playerItem.RemainingExploitEfficiency}/{playerItem.Item.ExploitEfficiency}");
            }
            if (playerItem.Item.Security > 0)
            {
                ConsoleWriter.WriteText($"\t\tSecurity: {playerItem.RemainingSecurity}/{playerItem.Item.Security}");
            }
        ...
        public void ShowStats(int playerId)
        {
            ...
            {
                ConsoleWriter.WriteText($"[{player.CurrentEnergyPlayerItem.Item.Name}] ", ConsoleColor.Yellow, false);
                ConsoleWriter.WriteText($"{player.CurrentEnergyPlayerItem.RemainingEnergy}/{player.CurrentEnergyPlayerItem.Item.Energy}  ", null, false);
            }
            else
            {
            ...
            //Check exploitEfficiency consumption
            if (player.CurrentExploitEfficiencyPlayerItem != null)
            {
                ConsoleWriter.WriteText($"[{player.CurrentExploitEfficiencyPlayerItem.Item.Name}] ", ConsoleColor.Yellow, false);
                ConsoleWriter.WriteText($"{player.CurrentExploitEfficiencyPlayerItem.RemainingExploitEfficiency}/{player.CurrentExploitEfficiencyPlayerItem.Item.ExploitEfficiency}  ", null, false);
            }
            else
            {
                ConsoleWriter.WriteText("[ExploitEfficiency] ", ConsoleColor.Red, false);
                ConsoleWriter.WriteText("nothing ", null, false);
            }
            ...
            {
                ConsoleWriter.WriteText($"[{player.CurrentSecurityPlayerItem.Item.Name}] ", ConsoleColor.Yellow, false);
                ConsoleWriter.WriteText($"{player.CurrentSecurityPlayerItem.RemainingSecurity}/{player.CurrentSecurityPlayerItem.Item.Security}  ", null, false);
            }
            else
            {
                ConsoleWriter.WriteText("[Security] ", ConsoleColor.Red, false);
                ConsoleWriter.WriteText("nothing ", null, false);
            }
